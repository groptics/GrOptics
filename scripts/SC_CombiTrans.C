...
// rotate and translate volumes via TGeoCombiTrans. Only for 
// primary and secondary.  Test bed for rotational error and 
// translational error offsets 
// Charlie Duke, Grinnell College, Feb. 29, 2012
...
void SC()
{
  ...
  TGeoCombiTrans *cPrim = new TGeoCombiTrans("cPrim",
					      ...
					      0.0*mm,
					      0.0*mm,rPrim);
  top->AddNode(primaryMirror, 1,rPrim);

  //Double_t fZs = kZs;
  // Make the secondary mirror
  AGeoAsphericDisk* secondaryV = new AGeoAsphericDisk("secondaryV", 0.0, 0, 0.0 + 1*um, 0, kDs/2., 0*m); // curvatures are set to infinity (0 = 1./inf)
  secondaryV->SetPolynomials(6, kVi, 6, kVi);
  AMirror* secondaryMirror = new AMirror("secondaryMirror", secondaryV);
  secondaryV->InspectShape();
   ...
   TGeoCombiTrans *cSec = new TGeoCombiTrans("cSec",
					     ...
					     0.0*mm,
					     kZs,rSec);
   top->AddNode(secondaryMirror, 1,cSec);
